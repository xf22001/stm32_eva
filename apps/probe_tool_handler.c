 ...
 *   修改日期：2021年05月12日 星期三 11时18分00秒
 *   描    述：
 *
 *================================================================*/
...
static void fn12(request_t *request)
{
	...
	if((ret == 6) || (ret == 5)) {
		...
		request_ftp_client_download(ftp_server_path->host, ftp_server_path->port, ftp_server_path->path, ftp_server_path->user, ftp_server_path->password, NULL, NULL);
	}
